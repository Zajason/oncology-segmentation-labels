...
def _load_model(config):
    ...
    try:
        ...
        raise ImportError("Install torch and segmentation-models-pytorch to use unet.py.") from exc
...
def _prepare_crop(crop, size, torch, device):
    ...
    resized = cv2.resize(crop[:, :, :3], (size, size), interpolation=cv2.INTER_LINEAR)
...
def generate_masks(image, boxes, config):
    ...
    with torch.no_grad():
        for box in boxes:
            ...
            tensor = _prepare_crop(crop, size, torch, device)
            logits = model(tensor)
